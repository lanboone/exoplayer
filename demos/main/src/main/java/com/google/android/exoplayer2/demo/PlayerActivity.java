...
public class PlayerActivity extends AppCompatActivity
  ...
  private Button eqSelectBtn;
  private Button ctcChangeBtn;

  //private boolean ctc_status = true;

  EqAudioProcessor eqAudioProcessor = new EqAudioProcessor();
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    eqSelectBtn = findViewById(R.id.btn_change_eq);
    ...
    ModeDialog = new ModeDialog(this, new ModeDialog.Listener() {
      @Override
      public void onSelect(boolean bVolEffect, int mode) {
          Log.d("EqAudioProcessor","mode:"+mode+",isEnable:"+bVolEffect);


          if(mode == 1 || mode == 2 || mode == 3 /*|| mode == 4*/){

            if(bVolEffect == true ){
              EqAudioProcessorJni.getInstance().setMode(mode);
            }
          ...
          if(bVolEffect == true){
            isEnable = true;
            eqSelectBtn.setBackgroundResource(R.drawable.touping_2);
            eqAudioProcessor.enableAudioProcess();
            TrackSelectionParameters param =  player.getTrackSelectionParameters();
            ...
            player.setTrackSelectionParameters(
                param);
          }else if(bVolEffect == false){
            isEnable = false;
            eqSelectBtn.setBackgroundResource(R.drawable.touping);
            eqAudioProcessor.disableAudioProcess();
            TrackSelectionParameters param =  player.getTrackSelectionParameters();
            player.setTrackSelectionParameters(
                player.getTrackSelectionParameters()
                    .buildUpon()
            ...
            player.setTrackSelectionParameters(param);
          }

      }

      @Override
      public void isEnable(boolean bVolEffect) {
        Log.d("EqAudioProcessor","isEnable:"+bVolEffect);
        if(isEnalbe != bVolEffect){
          if(bVolEffect == true){
            isEnalbe = true;
            eqSelectBtn.setBackgroundResource(R.drawable.touping_2);
            eqAudioProcessor.enableAudioProcess();
      ...
      public void modeChange(int mode) {
        Log.d("EqAudioProcessor","mode:"+mode);
        if(mode == 1 || mode == 2 || mode == 3 || mode == 4 || mode == 5 || mode == 6){
          if(isEnalbe == true ){
            ...
            if(mode == 1){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode1Conf);
            }else if(mode == 2){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode2Conf);
            }else if(mode == 3){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode3Conf);
            }else if(mode == 4){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode4Conf);
            }else if(mode == 5){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode5Conf);
            }else if(mode == 6){
              EqAudioProcessorJni.getInstance().up(EqAudioProcessorJni.getInstance().mode6Conf);
            }
            EqAudioProcessorJni.getInstance().mMode =  mode;
          }
        }
      }
    });
    EqAudioProcessorJni.getInstance().updateConf();
  }
  ...
  private boolean isEnalbe = false;
  private EqSelectDialog eqDiaglog = new EqSelectDialog();
  ...
  public void onClick(View view) {
    if (view == selectTracksButton
      ...
      mParamDialog.show();
      /*
      if(ctc_status == false && isEnalbe == true){
        ctc_status = !ctc_status;
        ctcChangeBtn.setText("CTC OFF");
        EqAudioProcessorJni.getInstance().CtcValid(1);
      }
      else if(ctc_status == true){
        ctc_status = !ctc_status;
        ctcChangeBtn.setText("CTC ON");
        EqAudioProcessorJni.getInstance().CtcValid(0);
      }*/
  ...
  private void setRenderersFactory(
    ...
    RenderersFactory renderersFactory =
        DemoUtil.buildRenderersFactory(/* context= */ this, preferExtensionDecoders,eqAudioProcessor);
